...
internal static class ExtMatch
{
    ...
    public static MethodDefinition CreateMethodDefinition(this Parsing.Statements.Match match,
        CSharpOptions cSharpOptions, int skip, string typeName, string stateTypeName)
    {
        var localsCount = 0;
            ...
            switch (reference.Referred)
            {
                ...
                case Parsing.Production production:
                {
                    methodDefinition.Add(new IfPart(IfPart.EIfScope.If,
                        string.Concat(production.ToCppCanMatchMethodName(cSharpOptions),
                ...
                {
                    methodDefinition.Add(new IfPart(IfPart.EIfScope.If,
                        string.Concat(leftRecursive.ToCppCanMatchMethodName(cSharpOptions),
                            $@"({Constants.DepthVariable} + 1)"))
                    {
                        $@"skip();"
                    });
        ...
        foreach (var reference in references)
        {
            ...
            {
                var captureDefinition = cSharpOptions.ClassCaptureDefinitionsMap.GetValueOrDefault(reference) ??
            ...
            else
            {
                switch (reference.Referred)
                {
                    ...
                    case Parsing.Token token:
                        ...
                        break;
                    case Parsing.Production production:
                        methodDefinition.Add(string.Concat(production.ToCppMatchMethodName(cSharpOptions),
                            $@"({Constants.DepthVariable} + 1);"));
                        break;
                    case Parsing.LeftRecursive leftRecursive:
                        methodDefinition.Add(string.Concat(leftRecursive.ToCppMatchMethodName(cSharpOptions),
                            $@"({Constants.DepthVariable} + 1);"));
                        break;
        ...
        methodDefinition.AddRange(match.Children.Handle(cSharpOptions, false, ToUnique));
