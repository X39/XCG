...
internal static class ExtIf
{
    ...
    /// <returns></returns>
    public static MethodDefinition CreateMethodDefinition(this Parsing.Statements.If @if, CSharpOptions cSharpOptions, string typeName, string stateTypeName)
    {
        var localsCount = 0;
        string ToUnique(string str) => string.Concat(str, (++localsCount).ToString());
        var resettable = ToUnique("resettable");

        var ifName = cSharpOptions.ToUnique("if");
        ...
        methodDefinition.AddRange(@if.Condition!.GetEvaluationResult(cSharpOptions, stateTypeName, conditionVariable, true, ToUnique));
        ...
        foreach (var isCan in Constants.TrueFalseArray)
        {
            ...
            ifPart.AddRange(@if.Children.Handle(cSharpOptions, isCan, ToUnique));
            ...
            if (@if.Else.Any())
            {
                ...
                elsePart.AddRange(@if.Else.Handle(cSharpOptions, isCan, ToUnique));
